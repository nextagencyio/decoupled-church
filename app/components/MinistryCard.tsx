...
export default function MinistryCard({ item }: MinistryCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
        {(item as any).leaderName && (
          <p className="text-sm text-primary-600 font-medium mb-2 uppercase tracking-wider">{(item as any).leaderName}</p>
        )}
