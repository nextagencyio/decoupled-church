...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Event Not Found | Church' }
  }
              ...
              <div className="bg-white border-l-4 border-accent-500 shadow-sm p-6 sticky top-8">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).registrationUrl && (
                    <div>
                      <dt className="text-sm text-gray-500 uppercase tracking-wider">Registration URL</dt>
                      <dd className="font-medium text-gray-900 mt-1">{(item as any).registrationUrl}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
